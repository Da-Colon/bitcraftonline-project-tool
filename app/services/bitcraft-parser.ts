...
import { BitCraftItem, BitCraftRecipe, ParsedItem, ParsedRecipe, ParsedRecipeInput } from '../types/bitcraft-data';
...
export function parseRecipes(bitCraftRecipes: BitCraftRecipe[]): ParsedRecipe[] {
  return bitCraftRecipes.map(parseRecipe).filter((recipe): recipe is ParsedRecipe => recipe !== null);
}
