...
async function fetchJson<T = unknown>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  try {
    ...
    if (!res.ok) {
      throw new BitJitaHttpError(`Upstream error ${res.status}`, { status: res.status, url, body: text });
    }
...
export async function searchPlayers(q: string): Promise<Player[]> {
  ...
  if (data && typeof data === "object") {
    if (Array.isArray((data as any).players)) return PlayerSchema.array().parse((data as any).players);
