import type { PlayerInventories, ClaimInventoriesResponse, Inventory, InventoryItem, ClaimInventory } from "~/types/inventory";
...
export function combineAllTrackedInventories(
  ...
  const trackedInventories = allInventories.filter(inv => 
    trackedInventoryIds.has(inv.id)
  );
